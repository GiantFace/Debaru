...
export function Select({ label, optional, options, value, placeholder = 'Válasszon…', onChange }) {
    ...
    <div className="field group">
      ...
      <div className={`select${open ? ' open' : ''}`} ref={ref}>
        <button type="button" className="select-btn" aria-haspopup="listbox" aria-expanded={open} onClick={() => setOpen((o) => !o)}>
          <span className={value ? 'select-val' : 'select-ph'}>{value || placeholder}</span>
        ...
        {open && (
          <ul className="select-menu" role="listbox">
            {value && <li className="select-opt select-clear" role="option" aria-selected={false} onClick={() => pick('')}>Nincs megadva</li>}
            {options.map((o) => (
